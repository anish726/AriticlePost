...
import classes from './UserPostArticleGet.module.css'
import { useSelector, useDispatch } from 'react-redux'
import { userPostArticleDelete } from '../../../redux/action/UserPostArticleSet';



const UserPostArticleGet = () => {
    const [readMore ,setReadMore] = useState(false);
    
 
    const userPostArticleGet = useSelector(state => state.userPostArticleSet.userPostArticleGet);
    console.log(userPostArticleGet, "hey  hey getall");
   ...
   const dispatch = useDispatch();
    ...
    return (
        <>
            ...
            {userPostArticleGet?.map((item,id) => {
                return (
                    <article className={classes.card} key={item.id}>

                        <div className={classes.title}>
                            <i className="fa fa-trash" onClick={()=>dispatch(userPostArticleDelete(item.id))}></i>
                        {/* <Link to={`/editForm/${item.id}`}><i className="fa fa-edit" ></i></Link> */}
...
export default UserPostArticleGet